...
//                             <div className="form-actions">
//                                 <button type="submit" className="btn btn-primary">{editing ? 'Update' : 'Add Certificate'}</button>
//                                 {editing && <button type="button" className="btn btn-outline" onClick={() => { setEditing(null); setCertForm({ title: '', issuer: '', date: '', image: '' }); }}>Cancel</button>}
//                             </div>
//                         </form>
//                         <div className="items-list">
//                             {certificates.length === 0 && <p className="muted">No certificates yet.</p>}
...
//                                     </div>
//                                     <div className="item-actions">
//                                         <button className="btn btn-outline" onClick={() => { setCertForm({ title: c.title, issuer: c.issuer, date: c.date, image: c.image }); setEditing(c._id); }}>Edit</button>
//                                         <button className="btn-danger" onClick={() => del(`/certificates/${c._id}`)}>Delete</button>
//                                     </div>
//                                 </div>
//                             ))}
            ...
            <div className="admin-body">
                ...
                {tab === 'About' && (
                    <form className="admin-form card" onSubmit={saveAbout}>
                        ...
                        <div className="form-row">
                            <label>About Section Image URL
                                <input value={about.aboutImage || ''} onChange={e => setAbout({ ...about, aboutImage: e.target.value })} placeholder="Portfolio About section photo URL..." />
                            </label>
                        </div>
                        <label>Resume URL<input value={about.resumeUrl || ''} onChange={e => setAbout({ ...about, resumeUrl: e.target.value })} /></label>
                        <button type="submit" className="btn btn-primary">Save Changes ✨</button>
                ...
                {tab === 'Skills' && (
                    <>
                        <form className="admin-form card" onSubmit={saveSkill}>
                            <h3>{editing ? '✏️ Edit Skill' : '➕ Add Skill'}</h3>
                            <div className="form-row">
                                <label>Name<input value={skillForm.name} onChange={e => setSkillForm({ ...skillForm, name: e.target.value })} required placeholder="e.g. React, Node.js, MongoDB" /></label>
                                <label>Category
                                    <select value={skillForm.category} onChange={e => setSkillForm({ ...skillForm, category: e.target.value })}>
                            ...
                            <div className="form-actions">
                                <button type="submit" className="btn btn-primary">{editing ? 'Update' : 'Add Skill'}</button>
                                {editing && <button type="button" className="btn btn-outline" onClick={() => { setEditing(null); setSkillForm({ name: '', category: 'Frontend', level: 80 }); }}>Cancel</button>}
                            </div>
                        </form>
                        <div className="items-list">
                            {skills.map(s => (
                                <div key={s._id} className="list-item card">
                                    <div className="item-info">
                                        <div>
                                            ...
                                            <p className="muted">{s.category} — {s.level}%</p>
                                        </div>
                                    </div>
                                    <div className="item-actions">
                                        <button className="btn btn-outline" onClick={() => { setSkillForm({ name: s.name, category: s.category, level: s.level }); setEditing(s._id); }}>Edit</button>
                                        <button className="btn-danger" onClick={() => del(`/skills/${s._id}`)}>Delete</button>
                                    </div>
                                </div>
                            ))}
